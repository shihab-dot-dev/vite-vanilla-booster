...
try {
    try {
        function setDataLayer() {
            ...
            if (typeof (partnerApp) != "undefined" && (partnerApp) !== null) {
                console.log('account type : partnerApp obj is defined');
                if (typeof (partnerApp.loginToken) != "undefined" && (partnerApp.loginToken) !== null) {
                    console.log('account type : login token is there');
                    if (typeof (partnerApp.loginToken.accounts) != "undefined" && (partnerApp.loginToken.accounts) !== null) {
                        console.log('account type :  account is present');
                        if (typeof (partnerApp.loginToken.accounts.length) !== 0) {
                            console.log('account type : atleast one account is present');
                            if (typeof (partnerApp.loginToken.accounts[0].accTypeName) != "undefined" && (partnerApp.loginToken.accounts[0].accTypeName) !== null) {
                                console.log('account type : account type is set from the auth token');
                                var getCustomerTypeMetaInfo = JSON.parse(localStorage.getItem("userTypeInfo"));
                                
                        if(typeof getCustomerTypeMetaInfo != "undefined" && getCustomerTypeMetaInfo !== null){
                            console.log('account type : userTpeInfo localstorage is present');
                            const getCustomerTypeLabel = getCustomerTypeMetaInfo['labelText'];
                            const isCustomerATestUser = getCustomerTypeMetaInfo['isATestUser'];
                            
                            isCustomerATestUser ? a_accountType = getCustomerTypeLabel : a_accountType = partnerApp.loginToken.accounts[0].accTypeName;
                        } else {
                            console.error('account type : userTpeInfo localstorage is not present');
                        }
                            }
                        }
                    }
